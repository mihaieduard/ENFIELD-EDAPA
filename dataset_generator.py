...
def capture_images(client, tmp_dir, meshNames, num_iterations=20):
    """
    Capture and save images along with their annotations.
    """
    height = [10, 20, 30, 40, 50, 60, 70, 80, 90]
    pitch_angle = [-0.3,  0,  0.3]
    y_pos = [-30, -20, -10]
    sun_poz = [200, 230, 300, 340]
    snow = [0, 50, 100]
    leaves = [0, 50]
    dust = [0, 20]
    fog = [0, 30]
    for h in height:
        for p in pitch_angle:
            for yp in y_pos:
                for x in range(num_iterations):

                    # Set the vehicle's pose

                    client.simSetVehiclePose(airsim.Pose(airsim.Vector3r(x + 90, yp, h*(-1)), airsim.to_quaternion(p, 0, 0)), True)
                    time.sleep(0.1)
                    ...
                    save_images_and_annotations(responses, dets, tmp_dir, meshNames, x, h, p, yp, sun_poz[1], snow[0], leaves[0], dust[1], fog[0])

                    client.simPause(is_paused=False)


def save_images_and_annotations(responses, dets, tmp_dir, meshNames, iteration, h, p, yp, sun_poz, snow, leaves, dust, fog):
    """
    Save captured images and their annotations.
    """
    image_file = os.path.normpath(os.path.join(tmp_dir, str(0), f"{iteration}_0_{h}_{p}_{yp}_{sun_poz}_{snow}_{leaves}_{dust}_{fog}"))
    ...
    for i, response in enumerate(responses):
        if response.pixels_as_float:
            ...
            if response.image_type == ImageType.Scene:
                image_type_str = ".jpg"
